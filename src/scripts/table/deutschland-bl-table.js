...
export function init(config) {
  ...
  const analysis = metaData.map(stateMeta => {
    ...
    return {
      ...
      'Fälle pro 100.000 <br>Einwohner (7 Tage)': `<span class="${trendArrow(weekTrend(stateCases))}" title="${pretty(weekTrend(stateCases) || 0)} %"></span> ${pretty(casesPer100Tsd7Days(stateCases, stateMeta.pop))}`,
      'Fälle': `<span class="${trendArrow(weekTrend(stateCases))}" title="${pretty(weekTrend(stateCases) || 0)} %"></span> ${pretty(currentCount(stateCases))} (+${pretty(currentIncrease(stateCases))})`,
      'Todesfälle': `<span class="${trendArrow(weekTrend(stateDeaths))}" title="${pretty(weekTrend(stateDeaths) || 0)} %"></span> ${pretty(currentCount(stateDeaths))} (+${pretty(currentIncrease(stateDeaths))})`
    };
