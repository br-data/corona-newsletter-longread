import { pretty, currentCount, currentIncrease, casesPer100Tsd7Days, json2table } from '../utils';
...
export function init(config) {
  ...
  const analysis = metaData.map(countyMeta => {
    ...
    //   'Landkreis/Stadt': countyMeta.name,
    ...
    return {
      'value': casesPer100Tsd7Days(countyCases, countyMeta.pop),
      'Landkreis/Stadt': countyMeta.name,
      '7-Tage-Inzidenz': `${pretty(casesPer100Tsd7Days(countyCases, countyMeta.pop))}`,
      'Fälle': `${pretty(currentCount(countyCases))} (${pretty(currentIncrease(countyCases), true)})`,
      'Todesfälle': `${pretty(currentCount(countyDeaths))} (${pretty(currentIncrease(countyDeaths), true)})`
    };
